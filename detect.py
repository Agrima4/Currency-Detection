...
def xyz():
    ...
    (kp1, des1) = orb.detectAndCompute(test_img, None)
    img3 = cv2.drawKeypoints(test_img,kp1,test_img,color=(0,255,0), flags=0)

    u.display('feature', img3)
    training_set = ['files/500.jpg']
    ...
    for i in range(0, len(training_set)):
        ...
        for (m, n) in all_matches:
            if m.distance < 0.987 * n.distance:
                good.append([m])
        ...
        total = len(good) / number_keypoint * 100
    ...
    if  total >= 80:
        ...
        print('How good is the match: ', total ,'%')
...
btn1=Button(root,text="Run",width=50,command=xyz)
